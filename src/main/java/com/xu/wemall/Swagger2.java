...
public class Swagger2 {
    ...
    //    // 定义分隔符,配置Swagger多包
    private static final String CONTROLLER_BASE_PATH = "com.xu.wemall.controller";

    private static final String CONTROLLER_WECHAT_PATH = CONTROLLER_BASE_PATH + ".weixin";
     ...
     */
    @Bean
    public Docket restWechatApi() {
        return new Docket(DocumentationType.SWAGGER_2)
                .groupName("微信接口")
                .apiInfo(apiInfo("【微信接口】Spring Boot中使用Swagger2构建RESTful APIs", "1.0"))
                .useDefaultResponseMessages(true)
                .forCodeGeneration(false)
                .select()
                .apis(RequestHandlerSelectors.basePackage(CONTROLLER_WECHAT_PATH))
...
//                .select()
//                .apis(RequestHandlerSelectors.basePackage(CONTROLLER_BACKEND_PATH))//                .apis(RequestHandlerSelectors.withClassAnnotation(RestController.class))
//                .apis(RequestHandlerSelectors.withClassAnnotation(RestController.class))
//                .paths(PathSelectors.regex("/app.*"))
//                .paths(PathSelectors.ant("/common.*"))
//                .build();
