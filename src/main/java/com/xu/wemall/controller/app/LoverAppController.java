...
@Api(value = "情侣接口集合", tags = "情侣接口集合")
@RestController
@RequestMapping("/app/lover")
public class LoverAppController extends BaseController {
     ...
     * @return
     */
    @ApiOperation(value = "情侣分页列表", notes = "获取情侣分页列表")
    ...
    public ResultObject paging(String keyword, Integer pageNum, Integer pageSize) {
        ...
        try {
            ...
            //userQueryWrapper.lambda().eq(User::getStatus,StatusEnum.ON_LINE.getValue());
...
//            }
            ...
            log.info("分页查询数据：{}", JSONObject.toJSONString(userIPage));
    ...
    @ApiOperation(value = "情侣新增或者修改", notes = "情侣新增或者修改")
    @RequestMapping(value = "/saveOrUpdate", method = RequestMethod.POST)
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "header", dataType = "String", name = "Authorization", value = "token", required = false)
    }
    )
    public ResultObject saveOrUpdate(Lover Lover) {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    @ApiOperation(value = "查询情侣", notes = "查询情侣详情")
    @RequestMapping(value = "/detail", method = RequestMethod.GET)
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", dataType = "Integer", name = "id", value = "情侣ID", required = false),
            @ApiImplicitParam(paramType = "header", dataType = "String", name = "Authorization", value = "token", required = false)
    }
    )
